...
| Collaboration | Student record → Meetings & Notes: follow-up tasks, team notes with author-only edits and revision history, restricted records shown on request; My Tasks (profile menu, `tmi://tasks`); global search (toolbar, ⌘F); tapping a notification opens its record; `collaboration` UI fixture | `listStudentNotes`, `saveStudentNote`, `listRestrictedRecords` (every read audited), `createRestrictedRecord`, `listTasks`, `createTask`, `updateTask`, `listColleagues`, `searchWorkspace` (server-filtered by access); notifications for task assignment and forms awaiting review; rules make notes, tasks, and restricted records server-written only and restricted records unreadable directly | `collaboration.test.ts` (5), `search.test.ts` (5), `CollaborationTests` (6) |
