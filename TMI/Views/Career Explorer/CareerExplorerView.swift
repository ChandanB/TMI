...
struct CareerExplorerView: View {
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                // Animated background - reusing the existing BackgroundAnimationView
                ...
                VStack(spacing: 0) {
                    ...
                    enhancedSearchBar
                        ...
                        .animation(.spring(response: 0.6, dampingFraction: 0.7).delay(0.2), value: searchAppeared)
                    ...
                    enhancedCareerFieldPicker
                        ...
                        .animation(.spring(response: 0.6, dampingFraction: 0.7).delay(0.3), value: filtersAppeared)
                            ...
                            VStack(alignment: .leading, spacing: 16) {
                                if !filteredCareers.isEmpty {
                                    ...
                                    enhancedCareerStatsSummary
                                        ...
                                        .animation(.spring(response: 0.6, dampingFraction: 0.7).delay(0.4), value: statsAppeared)
                    ...
                    Button(action: {
                        ...
                        HStack(spacing: 6) {
                            ...
                            Image(systemName: "slider.horizontal.3")
                                ...
                                .background(
                                    Circle()
                                        .fill(hasActiveFilters ? Color.tmiSecondary.opacity(0.3) : Color.white.opacity(0.1))
                                        .background(
                                            Circle()
                                                .fill(.ultraThinMaterial)
                                                .opacity(0.3)
    ...
    struct BackgroundAnimationView: View {
        ...
        private let animationDelay: Double = 7.0
        ...
        var body: some View {
            ZStack {
                ...
                RadialGradient(
                    ...
                    center: .init(x: 0.5 + 0.5 * sin(phase), y: 0.5 + 0.5 * cos(phase)),
                    startRadius: 50,
                ...
                // Second animated gradient (offset phase for more complex animation)
                RadialGradient(
                    gradient: Gradient(colors: gradientColors.reversed()),
                    center: .init(x: 0.5 + 0.5 * sin(phase2), y: 0.5 + 0.5 * cos(phase2 * 0.8)),
                    startRadius: 100,
            ...
            .onAppear {
                // Start the animations with different timing to create more organic movement
                withAnimation(
                ...
                withAnimation(
                    Animation.linear(duration: animationDuration + animationDelay)
                        .repeatForever(autoreverses: false)
                ) {
                    phase2 = 2 * .pi
                }
    ...
    // Noise texture to add subtle grain effect
    struct NoiseTextureView: View {
        ...
        var body: some View {
            if let image = noiseImage {
                ...
                Color.clear
                    .onAppear(perform: generateNoiseImage)
            }
        }
        ...
        private func generateNoiseImage() {
            ...
            if let context = UIGraphicsGetCurrentContext() {
                ...
                for _ in 0..<10000 {
                    ...
                    let opacity = CGFloat.random(in: 0.05...0.2)
    ...
    struct FieldPickerButton: View {
        let field: String?  // nil represents "All Fields"
        let isSelected: Bool
        let iconName: String
        let action: () -> Void
        ...
        var body: some View {
            Button(action: action) {
                ...
                .shadow(color: isSelected ? Color.tmiSecondary.opacity(0.4) : Color.clear, radius: 8, x: 0, y: 4)
            }
            .buttonStyle(.plain)
        }
    }
            ...
            HStack(spacing: 12) {
                ...
                ForEach(Array(Set(careers.map { $0.field })).sorted(), id: \.self) { field in
                    FieldPickerButton(
                        ...
                        iconName: getFieldIcon(field: field),
                        action: {
                            withAnimation(.spring(response: 0.3, dampingFraction: 0.7)) {
    ...
    private var filteredCareers: [Career] {
        careers.filter { career in
            ...
            let matchesSkills = selectedSkills.isEmpty ||
                !selectedSkills.isDisjoint(with: Set(career.skills))
...
struct CareerCardBackground: View {
    ...
    var body: some View {
        RoundedRectangle(cornerRadius: 16)
            ...
            .shadow(color: isHovered ? Color.tmiSecondary.opacity(0.3) : Color.clear, radius: 10, x: 0, y: 5)
...
struct FilterActionButton: View {
    ...
    init(title: String, iconName: String? = nil, isPrimary: Bool = false, action: @escaping () -> Void) {
        self.title = title
    ...
    var body: some View {
        Button(action: action) {
            ...
            .shadow(color: isPrimary ? Color.tmiSecondary.opacity(0.4) : Color.clear, radius: 8, x: 0, y: 4)
        }
        .buttonStyle(ScaleButtonStyle())
    }
}
