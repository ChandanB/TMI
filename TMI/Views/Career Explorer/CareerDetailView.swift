...
struct CareerDetailView: View {
    ...
    private var relatedCareers: [Career] {
        Career.sampleCareers.filter { $0.field == career.field && $0.title != career.title }.prefix(3).map { $0 }
    }
                ...
                HStack {
                    ...
                    ForEach(0..<5) { i in
                        Image(systemName: "star.fill")
                            .foregroundColor(i < getGrowthRating(outlook: career.jobOutlook) ? .yellow : .gray.opacity(0.3))
                    }
                }
                .padding(.top, 8)
            }
                        ...
                        HStack(spacing: 16) {
                            ...
                            VStack(alignment: .leading, spacing: 4) {
                                ...
                                HStack {
                                    ...
                                    Spacer()
                                    Label(program.format, systemImage: program.format.contains("Online") ? "network" : "building.2")
                                }
                                .font(.caption)
                                .foregroundColor(.gray)
                            }
    ...
    private func getSkillResources() -> [(name: String, description: String, icon: String, color: Color)] {
        return [
            (
                name: "Online Courses",
    ...
    private func getEducationPath() -> [(level: String, description: String, timeline: String, icon: String, isRequired: Bool)] {
        return [
            (
                level: "High School",
    ...
    private func getRecommendedPrograms() -> [(name: String, institution: String, duration: String, format: String, initials: String)] {
        return [
            (
    ...
    private func getCareerPath() -> [(title: String, description: String, salary: String, yearsExperience: String, isCurrent: Bool)] {
        return [
            (
                title: "Entry Level",
                description: "Starting position focusing on basic responsibilities and learning the fundamentals",
