...
struct TMIPlanEditorView: View {
    ...
    private func autoUpdateTitle() {
        ...
        // Only auto-update if the current title looks auto-generated (starts with a model name) or is empty
        let matchesAnyModel = TMIPlanModel.allCases.contains { planTitle.hasPrefix($0.rawValue) }
        if planTitle.isEmpty || matchesAnyModel {
            planTitle = autoGeneratedTitle
        }
    }
