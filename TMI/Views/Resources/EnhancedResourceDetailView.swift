//
//  EnhancedResourceDetailView.swift
//  TMI
//
...
// Enhanced version of ResourceDetailView with career connections and improved metadata
    ...
    private var relatedResourcesSection: some View {
        TMIGlassCard(style: .elevated) {
            VStack(alignment: .leading, spacing: 16) {
                ...
                VStack(spacing: 12) {
                    ForEach(relatedResources.prefix(3), id: \.id) { resource in
                        NavigationLink(destination: EnhancedResourceDetailView(resource: resource)) {
                            CompactResourceRow(resource: resource)
                        }
                    }
                }
            }
...
#Preview {
    NavigationView {
        EnhancedResourceDetailView(resource: Resource.sampleResources.first!)
    }
}